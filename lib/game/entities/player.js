...
.defines(function(){ "use strict";
	...
	ig.EntityPlayer = ig.global.EntityPlayer = ig.Player.extend({
		...
		receiveTemperature: function (amount, from) {
			...
			if(temp.heat < temp.heatPivot - temp.heatLowOffsetThreshold) {
				var diff = temp.heatPivot - temp.heatLowOffsetThreshold;
				this.receiveDamage(2 + diff * 0.1, this, true);
			} else if(temp.heat > temp.heatPivot + temp.heatHighOffsetThreshold) {
				var diff = temp.heatHighOffsetThreshold - temp.heatPivot;
				this.receiveDamage(2 + diff * 0.1, this, true);
			}
		...
		updateTemperatureRegeneration: function() {
			...
			if (selectedEntities.length) {
				...
				selectedEntities.sort(function(a, b) {
					var h1 = a.getComponent('heat'), t2 = b.getComponent('heat');
					var d1 = a.distanceSquaredTo(this), d2 = b.distanceSquaredTo(this);
				...
				case this.STATE_NEUTRAL:
					...
					if (ig.input.pressed('attack')) {
						...
						if(entity != this && entity) {
							switch(entity.type) {
								case 384: // TODO: Figure out ig.EntityExtended.TYPE.CREATURE
									this.attack(entity);
									break;
							}
						}
