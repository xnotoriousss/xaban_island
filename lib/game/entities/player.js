...
.defines(function(){ "use strict";
	...
	ig.EntityPlayer = ig.global.EntityPlayer = ig.Player.extend({
		...
		updateTemperatureRegeneration: function() {
			...
			if (selectedEntities.length) {
				var sortedEntities = selectedEntities.sort(function(a, b) {
					...
					var d1 = a.distanceTo(self), d2 = b.distanceTo(self);
		...
		attack: function(creature) {
            ...
            if(creature) {
                if (this.equipment.items[0]) {
                    ig.combat.attack(creature, this, "melee", this.equipment.getWeaponData()['melee'].damage); // TODO: Replace 0 with weapon range
                } else {
                    ig.log("No Weapon");
		...
		harvest: function(resource) {
		    ...
		    if(resource) {
				if (!resource.requiredTool || (toolData && toolData.type === resource.requiredTool)) {
					resource.onHarvest(toolData.damage);
				} else {
		...
		use: function(slot) {
			...
			if (item) {
				var use = item.getComponents('use');
				if (use) {
					switch(use) {
						case "equip":
							this.switchContainer(slot, this.inventory, this.equipment);
						...
						case "build":
							...
							this.buildEntity = item.getComponent('build').type;
							this.buildSlot = slot;
							break;
						case "eat":
		...
		/*
		transfer: function(slot, entity) {
			var inv = this.inventory;
			var item = inv.getSlot(slot);
			if (entity && item) {
				var transfer = item.getComponent('transfer');
				var entityTransfer = entity.getComponent('transfer');
		...
		transferSelected: function(entity) {
			this.transfer(this.inventory.getSelectedSlot(), entity);
		},*/
			...
			switch(iState) {
				case this.STATE_NEUTRAL:
					...
					if (ig.input.pressed('inv_interact_1')) {
						...
						//this.inventory.transferSelected();
						this.inventory.logInfo();
					}
						...
						if(entity) {
							switch(entity.type) {
								case ig.EntityExtended.TYPE.RESOURCE:
									entity = _uti.entitiesInAABB(self.pos.x, self.pos.y,
														self.pos.x + self.size.x, self.pos.y + self.size.y,
														true, 'entities', false)[1];
									this.harvest(entity);
									break;
