    ...
    ig.ItemContainer = ig.global.ItemContainer = ig.Class.extend({
        ...
        addItem: function(item) {
            ...
            if(firstEmpty !== -1) {
				...
				ig.log("Item container too full to be added to");
			}
        ...
        getSlotIndexByName: function(name) {
            for(var i = 0, il = this.numSlots; i < il; i++) {
                if(this.items[i] && this.items[i].displayName === name) return i;
            }
		...
		getItemByName: function(name) {
			var slot = this.getSlotIndexByName();
			if (slot >= 0) {
				return this.items[this.getSlotIndexByName(name)];
			}
